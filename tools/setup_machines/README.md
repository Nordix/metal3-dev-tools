...
1. How do these components choose which interfaces to use? 
2. How do we influence the choice during init phase? 
3.  How do we influence the choice during join phase?
4. How granular the configuration could be made?
...
# Test Cases
1. Apply **kubeadm** init and join with default configurations to check how it populates the different static pod manifest files
2. Use a custom **kubeadm-init** config file where we add only **api-server** related configurations and check if a different interface for **api-server** is assigned
3. Extend the 2nd test case by adding **etcd-server** related configurations in **kubeadm-init** config file and check if different interface for **api-server**  and **etcd-server** gets assigned. 
4. In all of the above cases, consider the impact of these configuration using **kubeadm-join** config file.
