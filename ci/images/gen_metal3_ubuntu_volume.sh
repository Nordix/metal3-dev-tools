...
VM_NAME="${VM_PREFIX}-${VM_KEY}"
VM_PORT_NAME="${VM_NAME}-int-port"
BUILDER_VOLUME_NAME="${VM_PREFIX}-${VM_KEY}"
BASE_VOLUME_NAME="metal3-ubuntu"
VOLUME_SIZE="50"
CI_INSTALLER_VM_FLAVOR="2C-4GB-50GB"
...
# Copy provision_metal3_image.sh and reset_cloud_init.sh scripts to package installer VM
echo "Copying provision_metal3_image.sh and reset_cloud_init.sh scripts to package installer VM..."
scp \
...
echo "Executing provision_metal3_image.sh script..."
ssh \
  -o StrictHostKeyChecking=no \
  -o UserKnownHostsFile=/dev/null \
