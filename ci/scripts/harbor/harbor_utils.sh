...
# RESULT_NUM is a common argument and it specifies the maximum number of top
# level JSON objects in the JSON array that is returned after a API call.

# NOTE: Tags are not unique identifiers but digests are!

# NOTE: digests in the cleanup exclusion list file have to be in the following
# format sha256:<digest_number>
...
harbor_clean_OCI_repository() {
  local IMAGE PINFILE RETENTION_LIMIT DRY_RUN RESULT_NUM DIGEST_DATA
  ...
  DRY_RUN="${4:-true}"
  RESULT_NUM="${5:-50}"
  DIGEST_DATA="$(harbor_list_OCI_repository_digests \
    "${IMAGE}" "false" "${RESULT_NUM}")"
...
harbor_delete_OCI_artifact() {
  local IMAGE TAG DIGEST DRY_RUN AUTH REFERENCE

  IMAGE="${1:?}"
  TAG="${2:-""}"
  DIGEST="${3:-""}"
  DRY_RUN="${4:-true}"
  AUTH="${DOCKER_USER:?}:${DOCKER_PASSWORD:?}"
  REFERENCE="$(harbor_create_reference "${TAG}" "${DIGEST}")"

  if [[ "${DRY_RUN}" = "true" ]]; then
    echo "DRY_RUN: DELETE ${HARBOR_API}/${IMAGE}/artifacts/${REFERENCE}"
  else
    curl -s -H "${HEADER}" -X "DELETE" --basic -u "${AUTH}" \
      "${HARBOR_API}/${IMAGE}/artifacts/${REFERENCE}"
  fi
}

# Returns the digest numbers from an harbor repository in a clear text list
# format. The list of the digests is sorted in a descending order starting
# from latest image and ending with oldest image digest.
harbor_list_OCI_repository_digests() {
  local IMAGE DRY_RUN RESULT_NUM AUTH
  ...
  if [[ "${DRY_RUN}" = "true" ]]; then
    ...
    local RAW_DATA
    RAW_DATA="$(harbor_list_OCI_repository \
      "${IMAGE}" "${DRY_RUN}" "${RESULT_NUM}")"
    harbor_convert_json_to_list "${RAW_DATA}" \
      | jq '.[].digest' | sed -e 's/\"//g'
  fi
}
...
harbor_list_OCI_repository() {
  ...
  if [[ "${DRY_RUN}" = "true" ]]; then
    ...
    curl -s -H "${HEADER}" -X "GET" --basic -u "${AUTH}" \
      "${HARBOR_API}/${IMAGE}/artifacts?page_size=${RESULT_NUM}"
  fi
}
...
harbor_get_OCI_artifact() {
  local IMAGE TAG DIGEST DRY_RUN RESULT_NUM AUTH REFERENCE
...
harbor_get_digests_from_tag() {
  local IMAGE TAG DRY_RUN RESULT_NUM AUTH
  ...
  if [[ "${DRY_RUN}" = "true" ]]; then
    ...
    local RAW_DATA
    RAW_DATA="$(harbor_get_OCI_artifact \
      "${IMAGE}" \
      "${TAG}" \
      "" \
      "${DRY_RUN}" \
...
# as a result of a query the API might return a single JSON object instead of a
# list. In order to make the parsing logic simpler this function can be used to
# turn the results of a query.
