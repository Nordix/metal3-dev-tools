...
set_value() {
  ...
  if grep -q "${PARAMETER_NAME}" "${FILE}"; then
    sed -i "0,/.*${PARAMETER_NAME}.*/s//${VALUE}/" "${FILE}"
  else
    echo "${VALUE}" | sudo tee -a "${FILE}"
  fi
}
...
echo auth sufficient pam_unix.so likeauth nullok | sudo tee -a "${COMMON_PASS_PATH}"
echo password sufficient pam_unix.so remember=4 | sudo tee -a "${COMMON_PASS_PATH}"
echo /lib/security/$ISA/pam_cracklib.so retry=3 minlen=8 lcredit=-1 ucredit=-2 dcredit=-2 ocredit=-1 | sudo tee -a "${SYSTEM_AUTH_PATH}"

# Lock the account after five failed attempts
cat >> "${COMMON_PASS_PATH}" <<EOL
